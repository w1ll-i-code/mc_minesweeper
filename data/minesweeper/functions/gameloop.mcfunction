# LOOP
execute as @e[tag=field,tag=!bomb,tag=!update] at @s if block ~ ~1 ~ dirt_path run tag @s add update
execute as @e[tag=update,tag=!handled,scores={bomb_count=0}] at @s run fill ~1 ~1 ~1 ~-1 ~1 ~-1 dirt_path replace dirt
...
# LOOSE CONDITION
execute at @e[tag=bomb] if block ~ ~1 ~ dirt_path run tag @a add lost
...
# WIN CONDITION
execute as @e[tag=field] at @s if block ~ ~1 ~ dirt run scoreboard players add @a fields_left 1
execute as @e[tag=field] at @s if block ~ ~1 ~ farmland run scoreboard players add @a fields_left 1
